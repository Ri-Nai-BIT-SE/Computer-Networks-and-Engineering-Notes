...
*   **要求**: 
    *   为了确保在发送完一个帧之前能够检测到可能发生的最晚的冲突。
    *   帧的**发送时间 $T_f$ 必须不小于争用期 $2\tau$**。

*   **公式**:
    $$T_f \geq 2\tau$$
    
    $$\frac{\text{最小帧长}}{\text{数据传输速率}} \geq 2\tau$$
    
    $$\text{最小帧长} \geq \text{数据传输速率} \times 2\tau$$

*   **以太网标准**:
    *   经典以太网（10 Mbps）规定最小帧长为 **64 字节**（512 比特）。
    *   对于 2500 米的总线，$\tau \approx 25.6 \mu s$。
    *   $2\tau \approx 51.2 \mu s$。
    *   在 10 Mbps 速率下，$T_f = 512 / (10 \times 10^6) = 51.2 \mu s$。
    *   恰好满足 $T_f = 2\tau$。
...
当发生冲突后，站点需要等待一段时间才能重传。这个等待时间的计算采用**二进制指数退避算法**：

*   **算法步骤**:
    1.  定义**基本退避时间** = $2\tau$（争用期）。
    2.  第 $k$ 次重传（$k \leq 10$）时：
        *   从集合 $\{0, 1, 2, ..., 2^k - 1\}$ 中**随机选择**一个整数 $r$。
        *   等待时间 = $r \times 2\tau$。
    3.  当 $k > 10$ 时，固定从 $\{0, 1, ..., 1023\}$ 中选择。
    4.  当重传次数达到 **16 次**仍失败时，放弃发送，向上层报告错误。

*   **示例**:
    ```
    第1次冲突 (k=1): r ∈ {0, 1}         → 等待 0 或 1 个 2τ
    第2次冲突 (k=2): r ∈ {0, 1, 2, 3}  → 等待 0-3 个 2τ
    第3次冲突 (k=3): r ∈ {0, 1, ..., 7}→ 等待 0-7 个 2τ
    ...
    ```

*   **特点**:
    *   随着重传次数 $k$ 的增加，随机选择的范围**指数级增大**。
    *   从而降低了再次冲突的概率。
    *   体现了"**动态自适应**"的思想。
...
*   **信道利用率**:
    $$U = \frac{1}{1 + \frac{5\tau}{T_f}}$$
    
    其中 $T_f$ 是帧的发送时间，$\tau$ 是单程传播时延。
